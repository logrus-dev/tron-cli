...
Grid mode works without an API key by default however the rate limit are ridiculous so effectively you have to use a TronGrid API key now. Obtain one at [TronGrid](https://www.trongrid.io/) and provide it with `--tronGridApiKey` or `TRON_CLI_TRON_GRID_API_KEY`. The bundled seed-node list may become outdated; use `--endpoint <node>` to specify a missing public node explicitly, including its IP or hostname and optional port.
